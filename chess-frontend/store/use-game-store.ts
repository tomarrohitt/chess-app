...
export const useGameStore = create<GameStore>((set) => ({
    ...
    setQueue: (queueStatus, timeControl = null) => set({ queueStatus, queueTimeControl: timeControl }),
    ...
    handleGameState: (p) => set((state) => ({
        activeGame: {
            gameId: p.gameId,
            fen: p.fen,
            pgn: p.pgn ?? "",
            turn: p.turn,
            playerColor: p.playerColor || (state.user?.id === p.whiteId ? "w" : "b"),
            whiteId: p.whiteId,
            blackId: p.blackId,
    ...
    resetGame: () => set({ activeGame: null, gameOver: null, queueStatus: "idle", queueTimeControl: null, drawOffer: null, drawOfferSent: null }),
}));